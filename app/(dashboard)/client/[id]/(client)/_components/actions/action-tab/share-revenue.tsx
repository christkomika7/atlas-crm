...
import { share } from "@/action/client.action";
...
export default function ShareRevenue({ revenues, isLoading }: ShareRevenueProps) {
  ...
  const { access: modifyAccess } = useAccess("CLIENTS", "MODIFY");
  ...
  useEffect(() => {
    if (revenues) {
      const now = new Date();
      form.setValue('emails', [revenues.client.email]);
      form.setValue('recordId', revenues.client.id);
      form.setValue('companyId', company);
      form.setValue('filename', `Relevé ${revenues?.client.companyName} du ${formatDateToDashModel(revenues?.startDate || new Date())} au ${formatDateToDashModel(revenues?.endDate || new Date())}`)
