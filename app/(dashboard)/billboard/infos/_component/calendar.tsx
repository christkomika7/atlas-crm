...
const Calendar = ({
  ...
  // Fonction pour extraire la date (YYYY-MM-DD) à partir d'une chaîne datetime
  const extractDate = (dateString: string) => {
    return dateString.split("T")[0]; // Garde seulement la partie date
  };
  ...
  const isDateInRange = (
    ...
    const start = new Date(extractDate(startDate));
    const end = new Date(extractDate(endDate));
  ...
  const getDayData = (day: number, month: number, year: number) => {
    ...
    return params.filter((param) => {
      // Si startDate et endDate sont définis, vérifier l'intervalle
      if (param.startDate && param.endDate) {
        return isDateInRange(dateKey, param.startDate, param.endDate);
      }
      // Sinon, utiliser l'ancien système avec id
