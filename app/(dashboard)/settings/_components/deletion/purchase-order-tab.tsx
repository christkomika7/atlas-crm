...
export default function PurchaseOrderTab() {
    ...
    useEffect(() => {
        if (companyId) {
            mutateGetDeletions({ companyId, type: "PURCHASE_ORDERS" }, {
                onSuccess(data) {
                    if (data.data) {
    ...
    function update(action: "cancel" | "delete", id: string, recordId: string,) {
        if (companyId) {
            mutateUpdateDeletion({ id, type: "PURCHASE_ORDERS", action, recordId }, {
                onSuccess() {
                    mutateGetDeletions({ companyId, type: "PURCHASE_ORDERS" }, {
                        onSuccess(data) {
                            if (data.data) {
            ...
            <Table>
                <TableHeader>
                    <TableRow className="h-14">
                        ...
                        <TableHead className="font-medium text-center">Montant payé</TableHead>
                        <TableHead className="font-medium text-center">Rest due</TableHead>
