...
import { SalesByClientItem } from '@/types/company.types';
import ExportChart from '../chart/export-chart';

type SellClientProps = {
    isLoading: boolean;
    datas: SalesByClientItem[];
    currency: string;
}

export default function SellClient({ isLoading, datas, currency }: SellClientProps) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="border border-neutral-200  p-4 pr-12  rounded-xl">
                <ExportChart
                    items={chartItems}
                    currency={currency}
                    isLoading={isLoading}
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="h-14">
                            ...
                            <TableHead className="font-medium text-center">Nombre de factures</TableHead>
                            ...
                            <TableHead className="font-medium text-center">Dû</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {isLoading ? (
                            ...
                            datas.map((data) => (
                                <TableRow key={data.id} className="h-12">
                                    <TableCell className="text-center">
                                        {formatDateToDashModel(data.date)}
                                    </TableCell>
                                    <TableCell className="text-center">
                                        {data.name}
                                    </TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        {formatNumber(data.totalGenerated || 0)} {currency}
                                    </TableCell>
                                    <TableCell className="text-center">
                                        {formatNumber(data.totalPaid || 0)} {currency}
                                    </TableCell>
                                    <TableCell className="text-center">
                                        {formatNumber(data.totalRemaining || 0)} {currency}
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
