...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: data.emails,
            subject: data.subject || `Bon de commande n° ${company.documentModel?.purchaseOrderPrefix || PURCHASE_ORDER_PREFIX}-${generateAmaId(purchaseOrder.purchaseOrderNumber, false)} – ${company.companyName}`,
            html: data.message ?
                `
            <div>
                <p>${data.message}</p>
