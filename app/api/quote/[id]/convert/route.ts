...
export async function POST(req: NextRequest) {
    ...
    const lastQuoye = await prisma.quote.findFirst({
        where: { companyId: quote.companyId },
        orderBy: { quoteNumber: "desc" },
        select: { quoteNumber: true },
    });

    if (data.convertTo === "delivery-note") {
        const key = generateId();
        const deliveryNoteNumber = `${quote.company.documentModel?.deliveryNotesPrefix ?? DELIVERY_NOTE_PREFIX}-${lastQuoye?.quoteNumber || 1}`;
        ...
        const key = generateId();
        const invoiceNumber = `${quote.company.documentModel?.invoicesPrefix ?? INVOICE_PREFIX}-${lastQuoye?.quoteNumber || 1}`;
