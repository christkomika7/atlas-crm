...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mailOptions = {
            from: {
                name: supplier.company.companyName,
                address: process.env.EMAIL_USER as string,
                contentType: 'application/pdf',
            },
            to: data.emails,
            subject: data.subject || `Relevé du fournisseur ${supplier.companyName} du ${formatDateToDashModel(new Date(data.start || new Date()))} au ${formatDateToDashModel(new Date(data.end || new Date()))}`,
            html: data.message ?
                `
            <div>
                <p>${data.message}</p>
            ...
            ` :
                ...
                `
            <div>
                <p>Bonjour ${supplier?.firstname} ${supplier?.lastname},</p>
                <p>Veuillez trouver ci-joint votre relevé fournisseur de la date du ${formatDateToDashModel(new Date(data.start || new Date()))} au ${formatDateToDashModel(new Date(data.end || new Date()))}.</p>
                ...
                <span style="color: #000;">${supplier.company.companyName}</span><br>
                <span style="color: #000;">${supplier.company.phoneNumber}</span>
                </p>
            </div>
        `,
            attachments: [
