...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: data.emails,
            subject: data.subject || `Votre facture n° ${company.documentModel?.invoicesPrefix || INVOICE_PREFIX}-${generateAmaId(invoice.invoiceNumber, false)} – ${company.companyName}`,
            html: data.message ?
                `
            <div>
                <p>${data.message}</p>
