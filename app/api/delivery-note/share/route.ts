...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: data.emails,
            subject: data.subject || `Votre bon de livraison n° ${company.documentModel?.deliveryNotesPrefix || DELIVERY_NOTE_PREFIX}-${generateAmaId(deliveryNote.deliveryNoteNumber, false)} – ${company.companyName}`,
            html: data.message ?
                `
            <div>
                <p>${data.message}</p>
