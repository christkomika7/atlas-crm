...
export async function GET(req: NextRequest) {
  ...
  const groupedBySource = allTransactions.reduce(
    (acc, transaction) => {
      ...
      if (transaction.transactionType === "RECEIPT") {
        acc[sourceName].totalReceipts = acc[sourceName].totalReceipts.plus(
          new Decimal(transaction.amount.toString()),
        );
      } else {
        acc[sourceName].totalDisbursements = acc[
          sourceName
        ].totalDisbursements.plus(new Decimal(transaction.amount.toString()));
      }
