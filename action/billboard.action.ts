...
export async function all({ companyId }: { companyId: string }) {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_AUTH_URL!}/api/billboard/${companyId}`, {
            method: 'GET',
        });
...
export async function create(data: BillboardSchemaFormType) {
    try {
        ...
        formData.append("installationCost", data.billboard.installationCost);
        formData.append("maintenance", data.billboard.maintenance);
        ...
        formData.append("lessorName", data.lessor.lessorName);
        formData.append("lessorJob", data.lessor.lessorJob);
        formData.append("lessorPhone", data.lessor.lessorPhone);
        formData.append("lessorEmail", data.lessor.lessorEmail);
        formData.append("capital", data.lessor.capital);
        formData.append("rccm", data.lessor.rccm);
        formData.append("taxIdentificationNumber", data.lessor.taxIdentificationNumber);
        formData.append("lessorAddress", data.lessor.lessorAddress);

        // LESSOR - Représentant légal
        formData.append("representativeName", data.lessor.representativeName);
        formData.append("representativeContract", data.lessor.representativeContract);

        // LESSOR - Contrat
        formData.append("leasedSpace", data.lessor.leasedSpace);
        ...
        formData.append("paymentMethod", data.lessor.paymentMethod);
        formData.append("specificCondition", data.lessor.specificCondition);
