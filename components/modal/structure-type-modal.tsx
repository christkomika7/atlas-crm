...
export default function StructureTypeModal() {
    const companyId = useDataStore.use.currentCompany();

    const datas = useBillboardStore.use.structureTypes();
    const addData = useBillboardStore.use.addStructureType();
    const removeData = useBillboardStore.use.removeStructureType();
    ...
    const { mutate: muatateCreateElement, isPending: isCreatingElement } = useQueryAction<
        BaseSchemaType & { type: "structure-type" | "structure-type" | "structure-type", lessorSpace?: "private" | "public" },
    ...
    } = useQueryAction<{ id: string, type: "structure-type" | "structure-type" | "structure-type" }, RequestResponse<BaseType>>(
    ...
    function submit(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        ...
        muatateCreateElement(
            { name, companyId, type: "structure-type" },
            {
                onSuccess(data) {
                    if (data.data) {
                        addData(data.data);
    ...
    return (
        <Dialog open={open} onOpenChange={(e) => setOpen(e)}>
            <DialogTrigger asChild>
                ...
                >
                    <PlusCircle className="fill-white stroke-blue !w-6 !h-6" /> Type de structure
                </Button>
            </DialogTrigger>
            <DialogContent className="min-w-sm">
                <DialogHeader>
                    <DialogTitle>Ajouter un type de structure </DialogTitle>
