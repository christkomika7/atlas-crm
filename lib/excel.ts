...
export async function generateTransactionsExcel(
    ...
    transactions.forEach((tx) => {
        const row = sheet.addRow({
            ...
            infos: tx.infos || "-",
            htAmount: tx.amountType === "HT" ? `${formatNumber(tx.amount)}` : "",
            ttcAmount: tx.amountType === "TTC" ? `${formatNumber(tx.amount)}` : "",
            paymentType: tx.paymentType ? getPaymentModeLabel(tx.paymentType) : tx.source?.name === "Caisse" ? 'Espéce' : "-",
