export const businessSectors = [
    { id: 1, value: "administration_publique", label: "Administration publique" },
    { id: 2, value: "aerospatial", label: "Aérospatial" },
    { id: 3, value: "agriculture", label: "Agriculture" },
    { id: 4, value: "agroalimentaire", label: "Agroalimentaire" },
    { id: 5, value: "alimentaire", label: "Alimentaire" },
    { id: 6, value: "alimentation_boissons", label: "Alimentation & Boissons" },
    { id: 7, value: "assurance", label: "Assurance" },
    { id: 8, value: "audit", label: "Audit" },
    { id: 9, value: "automobile", label: "Automobile" },
    { id: 10, value: "banque", label: "Banque" },
    { id: 11, value: "banque_assurance", label: "Banque & assurance" },
    { id: 12, value: "btp", label: "BTP / Construction" },
    { id: 13, value: "chimie", label: "Chimie" },
    { id: 14, value: "commerce", label: "Commerce" },
    { id: 15, value: "commerce_international", label: "Commerce international" },
    { id: 16, value: "communication_publicite", label: "Communication / Publicité" },
    { id: 17, value: "conseil", label: "Conseil" },
    { id: 18, value: "construction", label: "Construction" },
    { id: 19, value: "distribution", label: "Distribution" },
    { id: 20, value: "divertissement_medias", label: "Divertissement & médias" },
    { id: 21, value: "eau_assainissement", label: "Eau & Assainissement" },
    { id: 22, value: "education", label: "Éducation" },
    { id: 23, value: "electronique_grand_public", label: "Électronique grand public" },
    { id: 24, value: "energies_renouvelables", label: "Énergies renouvelables" },
    { id: 25, value: "energie", label: "Énergie" },
    { id: 26, value: "environnement", label: "Environnement" },
    { id: 27, value: "evenementiel", label: "Événementiel" },
    { id: 28, value: "exploitation_forestiere", label: "Exploitation forestière" },
    { id: 29, value: "extraction_miniere", label: "Extraction minière" },
    { id: 30, value: "finance", label: "Finance" },
    { id: 31, value: "formation", label: "Formation" },
    { id: 32, value: "formation_professionnelle", label: "Formation professionnelle" },
    { id: 33, value: "hotellerie", label: "Hôtellerie" },
    { id: 34, value: "immobilier", label: "Immobilier" },
    { id: 35, value: "import_export", label: "Import / Export" },
    { id: 36, value: "industrie", label: "Industrie" },
    { id: 37, value: "industrie_extractive", label: "Industrie extractive" },
    { id: 38, value: "institutions", label: "Institutions" },
    { id: 39, value: "internet_digital", label: "Internet & Digital" },
    { id: 40, value: "juridique", label: "Juridique" },
    { id: 41, value: "logistique_transport", label: "Logistique & transport" },
    { id: 42, value: "marketing", label: "Marketing" },
    { id: 43, value: "materiel_informatique", label: "Matériel informatique" },
    { id: 44, value: "medias", label: "Médias" },
    { id: 45, value: "mines", label: "Mines" },
    { id: 46, value: "nettoyage", label: "Nettoyage" },
    { id: 47, value: "nettoyage_securite", label: "Nettoyage & sécurité" },
    { id: 48, value: "ong_associations", label: "ONG / Associations" },
    { id: 49, value: "peche", label: "Pêche" },
    { id: 50, value: "petrole_gaz", label: "Pétrole & gaz" },
    { id: 51, value: "pharmaceutique", label: "Pharmaceutique" },
    { id: 52, value: "pharmacie", label: "Pharmacie" },
    { id: 53, value: "publicite_marketing", label: "Publicité & marketing" },
    { id: 54, value: "ressources_humaines", label: "Ressources humaines" },
    { id: 55, value: "restauration", label: "Restauration" },
    { id: 56, value: "robotique", label: "Robotique" },
    { id: 57, value: "sante", label: "Santé" },
    { id: 58, value: "securite", label: "Sécurité" },
    { id: 59, value: "services_aux_entreprises", label: "Services aux entreprises" },
    { id: 60, value: "services_juridiques", label: "Services juridiques" },
    { id: 61, value: "soins_de_sante", label: "Soins de santé" },
    { id: 62, value: "technologies_information", label: "Technologies de l’information" },
    { id: 63, value: "technologies_it", label: "Technologies / IT" },
    { id: 64, value: "telecommunications", label: "Télécommunications" },
    { id: 65, value: "textile", label: "Textile" },
    { id: 66, value: "tourisme", label: "Tourisme" },
    { id: 67, value: "tourisme_hotellerie", label: "Tourisme & hôtellerie" },
    { id: 68, value: "transport", label: "Transport" },
    { id: 69, value: "autres", label: "Autres" },
];

export const discount = [
    { id: 0, value: "0%", label: "0%" },
    { id: 1, value: "5%", label: "5%" },
    { id: 2, value: "10%", label: "10%" },
    { id: 3, value: "15%", label: "15%" },
    { id: 4, value: "20%", label: "20%" },
    { id: 5, value: "25%", label: "25%" },
    { id: 6, value: "30%", label: "30%" },
    { id: 7, value: "40%", label: "40%" },
    { id: 8, value: "50%", label: "50%" },
    { id: 9, value: "75%", label: "75%" },
    { id: 10, value: "100%", label: "100%" },
];

export const rentalDurations = [
    { id: 0, value: "6_months", label: "6 mois" },
    { id: 1, value: "1_year", label: "1 an" },
    { id: 2, value: "2_years", label: "2 ans" },
    { id: 3, value: "3_years", label: "3 ans" },
    { id: 4, value: "5_years", label: "5 ans" },
]



export const paymentTerms = [
    { id: 1, value: "paiement_7_jours", label: "Paiement sous 7 jours", data: 7 },
    { id: 2, value: "paiement_15_jours", label: "Paiement sous 15 jours", data: 15 },
    { id: 3, value: "paiement_30_jours", label: "Paiement sous 30 jours", data: 30 },
    { id: 4, value: "paiement_45_jours", label: "Paiement sous 45 jours", data: 45 },
    { id: 5, value: "paiement_60_jours", label: "Paiement sous 60 jours", data: 60 },
    { id: 6, value: "paiement_90_jours", label: "Paiement sous 90 jours", data: 90 },
    { id: 7, value: "paiement_120_jours", label: "Paiement sous 120 jours", data: 120 },
];


export const colors = [
    { id: 1, color: "#3366FF" }, { id: 2, color: "#33CC66" }, { id: 3, color: "#FF9933" }, { id: 4, color: "#FF4C4C" },
    { id: 5, color: "#6680FF" }, { id: 6, color: "#66CC99" }, { id: 7, color: "#FFB266" }, { id: 8, color: "#FF6666" },
    { id: 9, color: "#99AFFF" }, { id: 10, color: "#99D6B2" }, { id: 11, color: "#FFC299" }, { id: 12, color: "#FF8080" },
    { id: 13, color: "#B2CFFF" }, { id: 14, color: "#B2E0CC" }, { id: 15, color: "#FFD1B2" }, { id: 16, color: "#FF9999" },
    { id: 17, color: "#4D80FF" }, { id: 18, color: "#4DD08C" }, { id: 19, color: "#FFB380" }, { id: 20, color: "#FF6666" },
    { id: 21, color: "#6699FF" }, { id: 22, color: "#66E0AA" }, { id: 23, color: "#FFC9A3" }, { id: 24, color: "#FF8080" },
    { id: 25, color: "#80B3FF" }, { id: 26, color: "#80E0B3" }, { id: 27, color: "#FFD1B2" }, { id: 28, color: "#FF9999" },
    { id: 29, color: "#99CCFF" }, { id: 30, color: "#99E6CC" }, { id: 31, color: "#FFE0B3" }, { id: 32, color: "#FF4C4C" },
    { id: 33, color: "#B3D1FF" }, { id: 34, color: "#B3F0D6" }, { id: 35, color: "#FFE6CC" }, { id: 36, color: "#FF6666" },
    { id: 37, color: "#CCE0FF" }, { id: 38, color: "#CCFFE6" }, { id: 39, color: "#FFF0CC" }, { id: 40, color: "#FF8080" },
    { id: 41, color: "#E6F0FF" }, { id: 42, color: "#E6FFF5" }, { id: 43, color: "#FFF5E6" }, { id: 44, color: "#FF9999" },
    { id: 45, color: "#99B3FF" }, { id: 46, color: "#99FFCC" }, { id: 47, color: "#FFE6B3" }, { id: 48, color: "#FF4C4C" },
    { id: 49, color: "#6699FF" }, { id: 50, color: "#66CC99" },
];


export const priority = [
    {
        id: 1,
        color: "bg-red",
        label: "Urgent",
        value: "URGENT"
    },
    {
        id: 2,
        color: "bg-yellow-400",
        label: "Normal",
        value: "NORMAL"
    },
    {
        id: 3,
        color: "bg-blue-700",
        label: "Relax",
        value: "RELAX"
    },
];

export const transactions = [
    {
        id: 1,
        label: "Encaissement",
        value: "RECEIPT"
    },
    {
        id: 2,
        label: "Décaissement",
        value: "DISBURSEMENT"
    },
];



export const status = [
    {
        id: 1,
        color: "bg-black",
        label: "À faire",
        value: "TODO"
    },
    {
        id: 2,
        color: "bg-yellow-400",
        label: "En cour",
        value: "IN_PROGRESS"
    },
    {
        id: 3,
        color: "bg-emerald-500",
        label: "Terminé",
        value: "DONE"
    },
    {
        id: 4,
        color: "bg-red",
        label: "Bloqué",
        value: "BLOCKED"
    },
]

export const productServiceCategories = [
    {
        id: 1,
        label: "Digital",
        value: "Digital"
    },
    {
        id: 2,
        label: "Branding",
        value: "Branding"
    },
    {
        id: 3,
        label: "Conception",
        value: "Conception"
    },
    {
        id: 4,
        label: "Impression",
        value: "Impression"
    },
    {
        id: 5,
        label: "Autre",
        value: "Autre"
    },

]

export const productServiceUnitTypes = [
    {
        id: 1,
        label: "Unité",
        value: "Unit"
    },
    {
        id: 2,
        label: "Heures",
        value: "Hours"
    },
    {
        id: 3,
        label: "Jours",
        value: "Days"
    },
    {
        id: 4,
        label: "Mètre carré ",
        value: "Squaremeter"
    },
]

export const productServiceItemTypes = [
    {
        id: 1,
        label: "Produit",
        value: "PRODUCT"
    },
    {
        id: 2,
        label: "Service",
        value: "SERVICE"
    },
]

export const billboardStrucutures = [
    {
        id: 1,
        label: "Propre",
        value: "clean"
    },
    {
        id: 2,
        label: "Sale",
        value: "dirty"
    },
    {
        id: 3,
        label: "Endommagé",
        value: "damaged"
    },
    {
        id: 4,
        label: "Incomplet",
        value: "incomplete"
    },
    {
        id: 5,
        label: "Repeint",
        value: "paint"
    },
]

export const generalNotes = [
    {
        id: 1,
        label: "1",
        value: "one"
    },
    {
        id: 2,
        label: "2",
        value: "two"
    },
    {
        id: 3,
        label: "3",
        value: "three"
    },
    {
        id: 4,
        label: "4",
        value: "four"
    },
    {
        id: 5,
        label: "5",
        value: "five"
    },
];

export const lessorSpaceType = [
    {
        id: 1,
        label: "Privé",
        value: "private"
    },
    {
        id: 2,
        label: "Public",
        value: "public"
    },
];

export const recurrences = [
    {
        id: 1,
        label: "Non",
        value: "no",
    },
    {
        id: 2,
        label: "Tous les jours",
        value: "day",
    },
    {
        id: 3,
        label: "Toutes les semaines",
        value: "week",
    },
    {
        id: 4,
        label: "Toutes les 2 semaines",
        value: "2-week",
    },
    {
        id: 5,
        label: "Tous les mois",
        value: "month",
    },
    {
        id: 6,
        label: "Tous les 2 mois",
        value: "2-month",
    },
    {
        id: 7,
        label: "Tous les trimestres",
        value: "quarter",
    },
    {
        id: 8,
        label: "Tous les semestres",
        value: "semester",
    },
    {
        id: 9,
        label: "Tous les ans",
        value: "year",
    },
];

export const convert = [
    {
        id: 1,
        label: "Devis",
        value: "quote"
    },
    {
        id: 2,
        label: "Bon de livraison",
        value: "delivery-note"
    },
];


export const invoiceDuplicates = [
    {
        id: 1,
        label: "Devis",
        value: "quote"
    },
    {
        id: 2,
        label: "Bon de livraison",
        value: "delivery-note"
    }, {
        id: 3,
        label: "Facture",
        value: "invoice"
    },
];


export const paymentFrequency = [
    { id: 0, value: "monthly", label: "Mensuel" },
    { id: 1, value: "quarterly", label: "Trimestriel" },
    { id: 2, value: "semiannual", label: "Semestriel" },
    { id: 3, value: "annual", label: "Annuel" },
];


export const electricitySupply = [
    { id: 0, value: "yes", label: "Oui" },
    { id: 1, value: "no", label: "Non" },
];



export const acceptPayment = [
    {
        id: 1,
        label: "Espèces",
        value: "cash"
    },
    {
        id: 2,
        label: "Chèque",
        value: "check"
    },
    {
        id: 3,
        label: "Virement",
        value: "bank-transfer"
    },
]


export const lights = [
    {
        id: 1,
        label: "Filaire",
        value: "Filaire"
    },
    {
        id: 2,
        label: "Solaire",
        value: "Solaire"
    },
    {
        id: 3,
        label: "Non éclairé",
        value: "Non éclairé"
    },
]



export const reportTypes = [
    { id: 2, value: "salesByClient", label: "Ventes par client " },
    { id: 3, value: "salesByItem", label: "Ventes par produit|service" },
    { id: 4, value: "salesByBillboards", label: "Ventes par panneaux publicitaires" },
    { id: 5, value: "paymentsByDate", label: "Paiements par date" },
    { id: 6, value: "paymentsByClients", label: "Paiements par clients" },
    { id: 7, value: "paymentsByType", label: "Paiements par type" },
    { id: 8, value: "expensesByCategories", label: "Dépenses par catégories" },
    { id: 9, value: "expensesJournal", label: "Journal des dépenses" },
    { id: 10, value: "debtorAccountAging", label: "Âge des comptes débiteurs" },
];

export const periods = [
    { id: 1, value: "currentFiscalYear", label: "Exercice fiscal en cours" },
    { id: 2, value: "currentQuarter", label: "Trimestre en cours" },
    { id: 3, value: "currentMonth", label: "Mois en cours" },
    { id: 4, value: "previousMonth", label: "Mois précédent" },
    { id: 5, value: "previousYear", label: "Année précédente" },
];

export const movements = [
    { id: 1, value: "INFLOWS", label: "Entrée" },
    { id: 2, value: "OUTFLOWS", label: "Sortie" },
]
