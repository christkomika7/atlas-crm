...
export function calculateTaxesTotal({
  ...
  taxes,
  taxOperation = "cumul",
}: CalculateTaxesParamsTotal): CalculateTaxesResultTotal {
  const taxesResult: TaxResultTotal[] = [];
  let totalTax = 0;
  ...
  for (const tax of taxes) {
    ...
    if (tax.taxType === "HT") {
      ...
      taxPrice = baseAmount;
      // Recalculer le montant de la taxe pour TTC
      const totalRate = appliedRates.reduce((sum, rate) => {
        if (taxOperation === "sequence" && appliedRates.length > 1) {
          // Pour les taxes séquentielles, calculer le taux effectif total
          let effectiveRate = 0;
          let currentBase = 1;
          for (const r of appliedRates) {
            const singleTax = (currentBase * r) / 100;
            effectiveRate += singleTax;
            currentBase += singleTax;
          }
      ...
      const htPrice = baseAmount / (1 + totalRate);
      taxAmount = baseAmount - htPrice;
    } else {
      // Type indéfini : retourner le montant de base
      taxPrice = baseAmount;
  ...
  // Pour le total final, utiliser la logique HT par défaut
  const totalWithTaxes = parseFloat((totalPrice + totalTax).toFixed(2));
...
export async function urlToFile(path: string): Promise<File> {
  try {
    const url = `${process.env.NEXT_PUBLIC_AUTH_URL!}/api/upload?path=${encodeURIComponent(path)}`;
